...
export interface GitHubClient {
  ...
  compare(owner: string, repo: string, base: string, head: string): Promise<GitHubCompareResult>;
}
...
export function createGitHubClient(accessToken: string): GitHubClient {
  async function request<T>(path: string): Promise<T> {
    ...
    if (!res.ok) {
      throw new GitHubApiError(res.status, `GitHub API ${path} failed: ${res.status} ${await res.text()}`);
    }
  ...
  return {
    async getIdentity() {
      const data = await request<{ login: string; avatar_url: string; name: string | null }>("/user");
    ...
    async listBranches(owner, repo) {
      ...
      return data.map((b) => ({ name: b.name, commitSha: b.commit.sha, protected: b.protected }));
    },

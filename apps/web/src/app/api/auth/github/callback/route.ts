...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.redirect(`${appUrl}/dashboard?github_error=invalid_state`);
  }
  if (Date.now() - state.ts > 10 * 60 * 1000) {
    return NextResponse.redirect(`${appUrl}/dashboard?github_error=expired_state`);
  }
  ...
  try {
    ...
    await getAdminDb()
      ...
      .collection("private")
      .doc("github")
      .set({ accessToken, scope, login: identity.login, connectedAt: Date.now() });
  } catch (err) {
    const message = err instanceof Error ? err.message : "unknown_error";
    return NextResponse.redirect(`${appUrl}/dashboard?github_error=${encodeURIComponent(message)}`);
  }

  const redirectPath = state.projectId ? `/projects/${state.projectId}/github` : "/dashboard";
